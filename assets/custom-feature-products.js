...
(function() {
  ...
  function initIconButtons() {
    const iconBtns = document.querySelectorAll('.custom-feature-products__icon-btn[data-product-handle]');
  ...
  function closeAllDropdowns() {
    document.querySelectorAll('.fp-modal__size-select-wrapper.is-open').forEach(function(d) {
  ...
  function updateSelectedVariant() {
    ...
    if (!allOptionsSelected) {
      ...
      modalAddBtn.querySelector('.fp-modal__add-btn-text').textContent = 'ADD TO CART';
      return;
    }
    ...
    if (matchingVariant) {
      ...
      if (matchingVariant.available) {
        ...
        modalAddBtn.querySelector('.fp-modal__add-btn-text').textContent = 'SOLD OUT';
      }
    } else {
      modalVariantId.value = '';
      modalAddBtn.disabled = true;
  ...
  function addToCart() {
    ...
    if (cart && typeof cart.getSectionsToRender === 'function') {
      formData.append(
        'sections',
        cart.getSectionsToRender().map(function(section) { return section.id; })
      );
    ...
    fetch(cartAddUrl, config)
      ...
      .then(function(response) {
        ...
        if (shouldAddBundledProduct) {
          return addBundledProductToCart(softWinterJacketVariantId, cart).then(function(bundleResponse) {
  ...
  function addBundledProductToCart(bundleVariantId, cart) {
    ...
    return fetch(cartAddUrl, config)
      ...
      .then(function(response) {
        ...
        console.log('Bundled product (Soft Winter Jacket) added to cart automatically');
        return response;
      })
      .catch(function(error) {
